...
<div class="space-y-4">
  <div>
    ...
    <input type="range" min="0" max={Math.max(distanceState.maxDistance - 0.1, 0.1)} step="0.1" value={distanceState.minDistance} oninput={(e) => setMinDistance(parseFloat(e.target.value))} class="w-full" />
  ...
  <div>
    ...
    <input type="range" min={Math.max(distanceState.minDistance + 0.1, 0.2)} max="30" step="0.5" value={distanceState.maxDistance} oninput={(e) => setMaxDistance(parseFloat(e.target.value))} class="w-full" />
